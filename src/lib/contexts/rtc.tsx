...
/** Rtc mutators */
function mutators(setState: (value: RtcState | undefined) => void, session: SessionState) {
	return {
		/**
...
/** Rtc context state */
export type RtcContextState<Loaded extends boolean = true> = ((RtcState & { _exists: true }) | (Loaded extends true ? never : Partial<RtcState> & { _exists: false })) & { _mutators: RtcMutators };
...
export function RtcProvider({ children }: PropsWithChildren) {
	...
	return (
		<RtcContext.Provider value={{ ...state, _exists: state !== undefined, _mutators: mutators(setState, session) } as RtcContextState<false>}>
			{children}
		</RtcContext.Provider>
	);
}
