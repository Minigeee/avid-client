...
import {
  Media_ClientToServerEvents,
  Media_ServerToClientEvents,
} from '@/lib/types';
    ...
    async ({ kind, rtpParameters, appData }, callback, errback) => {
      ...
      try {
        _.socket.emit(
          ...
          (id) => {
            if (!id) errback(new Error('producer creation attempt denied'));
            else callback({ id });
          },
        );
      } catch (err) {
...
async function pauseConsumer(
  ...
  // Send event
  if (_.socket) _.socket.emit('consumers-paused', [consumer.id]);
...
async function resumeConsumer(
  ...
  // Send event
  if (_.socket) _.socket.emit('consumers-resumed', [consumer.id]);
...
async function deafen(emit: RtcSetState) {
  ...
  if (_state.joined) {
    ...
    // Send event
    if (_.socket) {
      if (paused.length > 0) _.socket.emit('consumers-paused', paused);
...
async function undeafen(emit: RtcSetState) {
  ...
  if (_state.joined) {
    ...
    // Send event
    if (_.socket) {
      if (resumed.length > 0) _.socket.emit('consumers-resumed', resumed);
...
async function enableWebcam(emit: RtcSetState, options?: WebcamOptions) {
  ...
  try {
    ...
    if (options.force_vp8) {
      codec = _.device.rtpCapabilities.codecs?.find(
    ...
    if (options.num_simulcast_streams && options.num_simulcast_streams > 0) {
      ...
      const firstVideoCodec = _.device.rtpCapabilities.codecs?.find(
        (c) => c.kind === 'video',
      );
    ...
    producer.on('trackended', () => {
      ...
      notification.info('Webcam', 'Your webcam has been disconnected.');
    });
...
async function enableShare(emit: RtcSetState, options?: VideoOptions) {
  ...
  try {
    ...
    producer.on('trackended', () => {
      ...
      notification.info('Stopped Sharing', 'Screen share has been stopped.');
    });
...
async function makeMediasoupDevice(
  capabilities: RtpCapabilities,
  producerConfig: TransportOptions,
  consumerConfig: TransportOptions,
) {
  // New device
  _.device = new Device();
...
function makeRtcSocket(
  ...
  _.socket.on('error', (message, status) => {
    if (status === 401) {
      ...
      notifyError(new Error(message), {
        ...
        message:
          'You are not authorized to perform the requested action. Please make sure you have the correct permissions.',
        notify: false,
      });
  ...
  _.socket.on(
    ...
    errorWrapper(
      async (
        participants: { id: string; is_admin: boolean; is_manager: boolean }[],
        callback: () => void,
      ) => {
        ...
        // Enable microphone
        if (!_state.is_mic_muted) enableMic(_state.audio_input_device, emit);
        // Enable webcam
        if (_state.is_webcam_on) enableWebcam(emit, _state.video_options);
  ...
  _.socket.on(
    'make-consumer',
    errorWrapper(
      async (options, callback: (success: boolean) => void) => {
  ...
  _.socket.on(
    ...
    errorWrapper(
      (participant_id: string) => {
        ...
        if (participants[participant_id]) delete participants[participant_id];
          ...
          participants: {
            ...
            [info.participant_id]: {
              ...
              [field]: {
                ...
                scalability: {
                  spatial: {
                    ...participant[field]?.scalability.spatial,
                    current: spatial,
                  },
                  temporal: {
                    ...participant[field]?.scalability.temporal,
                    current: temporal,
                  },
                },
              },
  ...
  _.socket.on(
    'consumer-score',
    errorWrapper(
      (consumer_id: string, score: ConsumerScore) => {
  ...
  _.socket.on(
    'producer-score',
    errorWrapper(
      (producer_id: string, score: ProducerScore) => {
  ...
  _.socket.on(
    'producer-locked',
    errorWrapper(
      (participant_id: string, type: MediaType) => {
  ...
  _.socket.on(
    ...
    errorWrapper(
      (participant_id: string, type: MediaType) => {
        ...
        if (participant_id === session.profile_id) {
          const newState = { ..._state };
          if (type === 'audio') newState.is_mic_locked = false;
          else if (type === 'video') newState.is_webcam_locked = false;
          else newState.is_share_locked = false;
...
function disconnect(emit: RtcSetState) {
  // Close socket
  if (_.socket?.connected) _.socket.disconnect();

  // Close send transport
  if (_.sendTransport && !_.sendTransport.closed) _.sendTransport.close();

  // Close receive transport
  if (_.recvTransport && !_.recvTransport.closed) _.recvTransport.close();
...
/** Rtc mutators */
function mutators(
  setState: (value: RtcState | undefined) => void,
  session: SessionState,
) {
  return {
    /**
    ...
    connect: async (room_id: string, domain_id?: string) => {
      ...
      if (_state?.joined) {
        if (_state.room_id !== room_id) disconnect(emit);
    ...
    webcam: {
      ...
      enable: async (options?: WebcamOptions) => {
        ...
        if (_state?.joined && !_state?.is_webcam_enabled)
          await enableWebcam((state) => {
            _state = state;
            setState(state);
          }, options || _state.video_options);
    ...
    audio: {
      ...
      setVolume: (participant_id: string, volume: number) => {
        ...
        if (
          volume <= 0 &&
          !_state.participants[participant_id].audio?.paused.local
        )
          pauseConsumer(
            (state) => {
              _state = state;
    ...
    microphone: {
      ...
      enable: (device_id?: string) =>
        enableMic(device_id || _state.audio_input_device, (state) => {
          _state = state;
          setState(state);
        }),
     ...
     */
    pause: (participant_id: string, type: MediaType) =>
      pauseConsumer(
        (state) => {
          _state = state;
          setState(state);
     ...
     */
    resume: (participant_id: string, type: MediaType) =>
      resumeConsumer(
        (state) => {
          _state = state;
          setState(state);
...
/** App state mutators */
export type RtcMutators = ReturnType<typeof mutators>;
/** Rtc context state */
export type RtcContextState<Loaded extends boolean = true> = (
  | (RtcState & { _exists: true })
  | (Loaded extends true ? never : Partial<RtcState> & { _exists: false })
) & { _mutators: RtcMutators };

/** Rtc context */
// @ts-ignore
export const RtcContext = createContext<RtcContextState<false>>();

/** Get rtc context state and mutators */
export function RtcProvider({ children }: PropsWithChildren) {
  ...
  return (
    <RtcContext.Provider
      value={
        {
          ...state,
          _exists: state !== undefined,
          _mutators: mutators(setState, session),
        } as RtcContextState<false>
      }
    >
      {children}
    </RtcContext.Provider>
