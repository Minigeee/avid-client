...
/** Acl entries wrapper */
export type AclEntriesWrapper<Loaded extends boolean = true> = SwrWrapper<
  AclEntry[],
  Loaded
>;
...
export function useAclEntries(resource_id: string | undefined) {
  return useApiQuery(
    resource_id ? `acl.by_resource.${resource_id}` : undefined,
    'GET /permissions',
    {
      query: {
...
export function useAclEntriesByRole(role_id: string | undefined) {
  return useApiQuery(
    role_id ? `acl.by_role.${role_id}` : undefined,
    'GET /permissions',
    {
      query: { role: role_id },
    },
...
export async function setAclEntries(
  domain_id: string,
  entries: (Omit<AclEntry, 'domain'> & { domain?: string })[],
  session: SessionState,
  _mutate: ScopedMutator,
) {
  try {
    // Patch permissions
    const { updated, deleted } = await api(
      'PATCH /permissions',
      {
        body: {
          domain: domain_id,
    ...
    for (const key of Array.from(hookSet)) {
      ...
      const unaccounted = new Set<string>(
        Object.keys(newMap).filter((x) => {
          const [resource, role] = x.split('.');
          return (
            (type === 'by_resource' && resource === group) ||
            (type === 'by_role' && role === group)
          );
        }),
      );
