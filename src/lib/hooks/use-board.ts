...
function _sanitize(board: Board) {
  for (const c of board.collections) _sanitizeCollection(c);

  return board;
}

////////////////////////////////////////////////////////////
function _sanitizeCollection(
  collection: Partial<TaskCollection>,
): Partial<TaskCollection> {
  if (collection.description)
    collection.description = sanitizeHtml(
...
function mutators(mutate: KeyedMutator<Board>, session?: SessionState) {
  ...
  return {
     ...
     * @returns The new board object
     */
    addCollection: (collection: NoId<TaskCollection>) =>
      mutate(
        swrErrorWrapper(
          async (board: Board) => {
            // Post new collection
    ...
    updateCollection: (
      ...
      collection: Partial<NoId<TaskCollection>>,
    ) =>
      mutate(
        swrErrorWrapper(
          async (board: Board) => {
      ...
      mutate(
        swrErrorWrapper(
          async (board: Board) => {
            ...
            if (idSet.size > 0) {
                ...
                (tasks: ExpandedTask[] | undefined) => {
                  ...
                  for (let i = 0; i < copy.length; ++i) {
                    if (idSet.has(copy[i].id))
                      copy[i] = {
                        ...copy[i],
                        collection: config.app.board.default_backlog.id,
                      };
                  }
                  return copy;
                },
              ...
              _mutate(
                (key) => typeof key === 'string' && idSet.has(key),
                (data: ExpandedTask | undefined) => {
                  if (!data) return data;
                  return {
                    ...data,
                    collection: config.app.board.default_backlog.id,
        ...
        ),
        {
          ...
          optimisticData: (board) => {
            ...
            return {
              ...board,
              collections: board.collections.filter(
     ...
     * @returns The new board object
     */
    addCollectionLocal: (collection: TaskCollection) =>
      mutate(
        swrErrorWrapper(
          async (board: Board) => {
            return {
              ...
              collections: [
                ...board.collections,
                _sanitizeCollection(collection),
              ],
            };
          },
          {
     ...
     * @returns The new board object
     */
    removeCollectionLocal: (collection_id: string) =>
      mutate(
        swrErrorWrapper(
          async (board: Board) => {
            return {
          ...
          {
            message:
              'An error occurred while displaying updated task collection',
          },
        ),
        { revalidate: false },
      ),
     ...
     * @returns The new board object
     */
    addTags: (options: { add?: Label[]; update?: WithId<Partial<Label>>[] }) =>
      mutate(
        swrErrorWrapper(
          async (board: Board) => {
            // Update tags
...
export type BoardWrapper<Loaded extends boolean = true> = SwrWrapper<
  Board,
  Loaded,
  BoardMutators
>;
