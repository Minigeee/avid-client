...
function mutators(mutate: KeyedMutator<Thread[]>, session: SessionState) {
  return {
     ...
     */
    setName: (thread_id: string, name: string) =>
      mutate(
        swrErrorWrapper(
...
export type ThreadsWrapper<Loaded extends boolean = true> = SwrWrapper<
  Thread[],
  Loaded,
  ThreadsMutators
>;
/** Swr data wrapper for threads without mutators */
export type ThreadsWrapperNoMutators<Loaded extends boolean = true> =
  | { _exists: true; data: Thread[] }
  | (Loaded extends true ? never : { _exists: false; data?: Thread[] });
...
export function setThreads(threads: Thread[], emit: boolean = true) {
  ...
  const copy = { ..._.store };
  for (const t of threads) copy[t.id] = t;
...
export function useThreadsById(
  ...
  return useMemo(() => {
    ...
    for (const id of ids) {
      ...
      if (!cached) needFetch.push(id);
      else threads.push(cached);
    }
