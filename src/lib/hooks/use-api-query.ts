...
>(
  ...
  path: Path,
  params: ApiReqParams<Path>,
  options: SwrApiQueryOptions<Path, In, Out, Mutators>,
) {
  ...
  const swr = useSWR<In | null>(
    ...
    () => {
      const promise = options.initial
        ? new Promise<ApiReturn<Path>>((resolve) => {
            resolve(options.initial as ApiReturn<Path>);
          })
        : api(path, params, { session });
