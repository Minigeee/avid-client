...
function _needFetch(
  ...
  lifetime: number = config.app.member.cache_lifetime,
) {
  return (
    (entry === undefined ||
      entry === null ||
...
/** Gets member entry */
function _getMemberEntry(
  domain_id: string,
  profile_id: string,
  cache: Record<string, MemberCache> = _.members,
) {
...
export function setMembers(
  ...
  options?: SetMembersOptions,
) {
  const now = Date.now();

  // Get domain cache
  let domain = _.members[domain_id];
  if (!domain) domain = _.members[domain_id] = {};
  else domain = { ...domain };

  // Set all members
  for (const member of members) {
    const newMember =
      options?.override_online === false && domain[member.id]
        ? { ...member, online: domain[member.id].data.online }
        : member;
...
export function setMemberQuery(
  domain_id: string,
...
export function useMemberMutators() {
  ...
  return {
     ...
     */
    addRoles: errorWrapper(
      async (domain_id: string, profile_ids: string[], role_id: string) => {
     ...
     */
    removeRole: errorWrapper(
      async (domain_id: string, profile_id: string, role_id: string) => {
        // Optimistic update
        const entry = _getMemberEntry(domain_id, profile_id);
        if (entry)
          // Set new member to cache
          setMembers(domain_id, [
            {
              ...entry.data,
              roles: entry.data.roles?.filter((r) => r !== role_id),
            },
          ]);
      ...
      },
      {
        ...
        onError: (
          ...
          if (entry)
            setMembers(domain_id, [
              { ...entry.data, roles: [...(entry.data.roles || []), role_id] },
            ]);
        },
      },
    ),
...
export type MemberWrapper<Loaded extends boolean = true> =
  | ({ _exists: true } & ExpandedMember)
  | (Loaded extends true
      ? never
      : { _exists: false } & Partial<ExpandedMember>);
...
export type MembersWrapper<Loaded extends boolean = true> =
  | { _exists: true; data: ExpandedMember[] }
  | (Loaded extends true ? never : { _exists: false; data?: ExpandedMember[] });
...
/** Multi member wrapper */
export type MemberQueryWrapper<Loaded extends boolean = true> =
  | ({ _exists: true } & MemberQueryResults)
  | (Loaded extends true
      ? never
      : { _exists: false } & Partial<MemberQueryResults>);
...
function _getQueryKey(domain_id: string, options?: MemberQueryOptions) {
  ...
  if (options?.role_id) extra.push(`role=${options.role_id}`);
  else if (options?.exclude_role_id)
    extra.push(`exclude_role=${options.exclude_role_id}`);
  if (options?.page) extra.push(`page=${options.page}`);
  if (options?.online !== undefined) extra.push(`online=${options.online}`);
  if (options?.no_data) extra.push(`no_data`);
...
function _filterEntries(entries: MemberEntry[], options?: MemberQueryOptions) {
  ...
  const filtered = entries
    .filter(
      (entry) =>
        (!search ||
          entry.data.alias.toLocaleLowerCase().indexOf(search) >= 0) &&
        (!options?.role_id ||
          (entry.data.roles &&
            entry.data.roles.indexOf(options.role_id) >= 0)) &&
        (!options?.exclude_role_id ||
          !entry.data.roles ||
          entry.data.roles.indexOf(options.exclude_role_id) < 0) &&
        (options?.online === undefined ||
          (entry.data.online || false) == options.online),
    )
    .sort((a, b) => a.data.alias.localeCompare(b.data.alias));

  return options?.page !== undefined
    ? filtered.slice(options.page * limit, (options.page + 1) * limit)
    : filtered;
}
...
export function useMemberQuery(
  ...
  return useMemo(() => {
    if (!domain_id) return { _exists: false } as MemberQueryWrapper<false>;
    ...
    let needFetch = _needFetch(
      queryKey,
      queryEntry,
      config.app.member.query_interval,
    );

    // Filter results and check for stales if query entry exists
    let filtered: MemberEntry[] = [];
    if (_exists) {
      filtered = _filterEntries(
        Object.values(_.members[domain_id] || {}),
        options,
      );
...
export function listMembersLocal(
  ...
  return _filterEntries(Object.values(_.members[domain_id] || {}), options).map(
    (x) => x.data,
  );
}
 ...
 */
export function updateMemberLocal(
  profile_id: string,
  fn: (member: ExpandedMember) => ExpandedMember,
  emit: boolean = true,
) {
 ...
 * @param emit Indicates if this change should be emitted
 */
export function updateMemberQueryLocal(
  filter: (domain_id: string, options: MemberQueryOptions) => boolean,
  fn: (count: number, domain_id: string, options: MemberQueryOptions) => number,
  emit: boolean = true,
) {
  ...
  for (const [key, entry] of Object.entries(copy)) {
    ...
    for (const opt of opts) {
      if (opt[0] === 'search') options.search = opt[1];
      else if (opt[0] === 'role_id') options.role_id = opt[1];
      else if (opt[0] === 'exclude_role_id') options.exclude_role_id = opt[1];
      else if (opt[0] === 'page') options.page = parseInt(opt[1]) || undefined;
      else if (opt[0] === 'online') options.online = opt[1] === 'true';
      else if (opt[0] === 'no_data') options.no_data = opt[1] === 'true';
    }
