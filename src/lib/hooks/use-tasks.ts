...
function _sanitize<T extends Partial<ExpandedTask>>(task: T) {
  if (task.description)
    task.description = renderNativeEmojis(
      sanitizeHtml(task.description, config.sanitize),
    );
...
////////////////////////////////////////////////////////////
function tasksMutators(
  mutate: KeyedMutator<ExpandedTask[]>,
  ...
  return {
     ...
     */
    addTask: (task: Partial<ExpandedTask> & { summary: string }) =>
      mutate(
        swrErrorWrapper(
          async (tasks: ExpandedTask[]) => {
        ...
        swrErrorWrapper(
          async (tasks: ExpandedTask[]) => {
            ...
            for (let i = 0; i < newTasks.length; ++i) {
              ...
              newTasks[i] = {
                ...task,
                subtasks: needsSubtaskFilter
                  ? task.subtasks?.filter((id) => !ids.has(id))
                  : task.subtasks,
                dependencies: needsDepFilter
                  ? task.dependencies?.filter((id) => !ids.has(id))
                  : task.dependencies,
              } as ExpandedTask;
            }
...
export type TasksWrapper<Loaded extends boolean = true> = SwrWrapper<
  ExpandedTask[],
  Loaded,
  TasksMutators
>;
...
export function useTasks(board_id: string | undefined, domain_id: string) {
  return useApiQuery(
    board_id ? `${board_id}.tasks` : undefined,
    'GET /tasks',
    {
    ...
    },
    {
      then: (results) => {
        ...
        return results.tasks.map((task) =>
          _sanitize({
            ...task,
            assignee: task.assignee ? results.members[task.assignee] : null,
          }),
        ) as ExpandedTask[];
      },
...
function taskMutators(
  ...
  return {
     ...
     */
    updateLocal: (task: Partial<ExpandedTask>) =>
      mutate(
        async (old) => {
          if (!old) return;
          return {
...
/** Swr data wrapper for a domain object */
export type TaskWrapper<Loaded extends boolean = true> = SwrWrapper<
  ExpandedTask,
  Loaded,
  TaskMutators
>;
