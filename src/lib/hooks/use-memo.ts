...
export function useMemoStateAsync<T>(key: string, factory: () => Promise<T>, dependencies?: DependencyList): [T | undefined, Dispatch<SetStateAction<T | undefined>>] {
	const [value, setValue] = useState<T | undefined>(_cache[key]);
	useEffect(() => {
		setTimeout(() => factory().then((v) => { setValue(v); _cache[key] = v }), 0) },
		dependencies
	);
	return [value, setValue];
}
