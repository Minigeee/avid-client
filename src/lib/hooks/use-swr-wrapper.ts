...
/** Wrapper options */
export type SwrWrapperOptions<In, Out, Mutators extends SwrMutators = {}, Infinite extends boolean = false, Separate extends boolean = false> = {
...
export function useSwrWrapper<In, Mutators extends SwrMutators = {}, Infinite extends boolean = false, Out = In, Separate extends boolean = false>
	...
	return useMemo(() => {
		...
		const swr = {
			...
			_next: infinite ?
				(n) => {
					...
					// @ts-ignore
					const isLoadingMore = response.isLoading || (response.size > 0 && response.data && !response.data[response.size - 1]);

					// @ts-ignore
					if (response.data[response.data.length - 1].length >= pageSize && !isLoadingMore)
						(response as SWRInfiniteResponse).setSize(((response as SWRInfiniteResponse).size || 0) + (n || 1));
				} : undefined,
		...
		const separate = Array.isArray(transformed) || typeof transformed !== 'object' || options?.separate;
