...
function _sanitize<T extends Partial<CalendarEvent>>(event: T) {
  if (event.description)
    event.description = renderNativeEmojis(
      sanitizeHtml(event.description, config.sanitize),
    );
...
function mutators(
  ...
  return {
     ...
     */
    addEvent: (event: Omit<CalendarEvent, 'id' | 'time_created'>) =>
      mutate(
        swrErrorWrapper(
...
export type CalendarEventsWrapper<Loaded extends boolean = true> = SwrWrapper<
  CalendarEvent[],
  Loaded,
  CalendarEventsMutators
>;
...
export function useCalendarEvents(
  ...
  const range = useMemo(() => {
    return [
      moment(date).startOf('year').subtract(1, 'month'),
      moment(date).endOf('year').add(1, 'month'),
    ];
  }, [date]);

  // Wrapper hook
  const wrapper = useApiQuery(
    channel_id ? `${channel_id}.calendar_events` : undefined,
    'GET /calendar_events',
    {
      query: {
        channel: channel_id || '',
    ...
    },
    {
      then: (events) => {
        ...
        if (!_cache[channel_id]) _cache[channel_id] = {};
        for (const ev of events) _cache[channel_id][ev.id] = _sanitize(ev);
        ...
        for (const ev of Object.values(_cache[channel_id])) {
          if (
            !idSet.has(ev.id) &&
            moment(ev.start).isAfter(range[0]) &&
            ev.end &&
            moment(ev.end).isBefore(range[1])
          )
            deleted.push(ev.id);
        }

        for (const id of deleted) delete _cache[channel_id][id];
  ...
  useEffect(() => {
    ...
    if (dt / 1000 > config.swr.dedupe_interval) wrapper._refresh();
  }, [range[0].toISOString()]);
...
export type CalendarEventWrapper<Loaded extends boolean = true> = SwrWrapper<
  CalendarEvent,
  Loaded
>;
