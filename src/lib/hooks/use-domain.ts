...
////////////////////////////////////////////////////////////
function mutators(
  mutate: KeyedMutator<ExpandedDomain>,
  ...
  return {
     ...
     */
    addChannel: <T extends ChannelTypes>(
      name: string,
      type: T,
      group_id: string,
      data?: ChannelData<T>,
      ...
      mutate(
        swrErrorWrapper(
          async (domain: ExpandedDomain) => {
            ...
            groups[idx] = {
              ...groups[idx],
              channels: [...groups[idx].channels, channel.id],
            };
      ...
      mutate(
        ...
        ),
        {
          ...
          optimisticData: (domain) => {
            if (!domain)
              throw new Error(
                'trying to rename channel from domain that is undefined',
              );
      ...
      mutate(
        swrErrorWrapper(
          async (domain: ExpandedDomain) => {
            ...
            const groups = domain.groups.map((group) => ({
              ...group,
              channels: group.channels.filter((c) => c !== channel_id),
            }));
        ...
        ),
        {
          ...
          optimisticData: (domain) => {
            if (!domain)
              throw new Error(
                'trying to remove channel from domain that is undefined',
              );
    ...
    moveChannel: (
      channel_id: string,
      from: { group_id: string; index: number },
      to: { group_id: string; index: number },
    ) => {
      function modifyGroups(domain: ExpandedDomain) {
        ...
        const dstIdx = same
          ? srcIdx
          : domain.groups.findIndex((x) => x.id === to.group_id);
        ...
        const dstChannels = same
          ? srcChannels
          : domain.groups[dstIdx].channels.slice();
     ...
     * @returns The new domain object
     */
    addGroup: (name: string, allow_everyone: boolean = true) =>
      mutate(
        swrErrorWrapper(
          async (domain: ExpandedDomain) => {
            // Create group
     ...
     * @returns The new domain object
     */
    setIcon: (image: Blob, fname: string) =>
      mutate(
        swrErrorWrapper(
          async (domain: ExpandedDomain) => {
     ...
     */
    updateRoles: (options: {
      changed?: Record<string, Partial<Role>>;
      order?: string[];
    }) =>
      mutate(
        swrErrorWrapper(
          async (domain: ExpandedDomain) => {
            ...
            if (options.changed && Object.keys(options.changed).length > 0) {
              ...
              const results = await api(
                ...
                {
                  body: {
                    roles: Object.entries(options.changed).map(
                      ([id, role]) => ({ ...role, id }),
                    ),
                  },
                },
              ...
              for (const role of results) map[role.id] = role;

              for (let i = 0; i < copy.length; ++i) {
                if (map[copy[i].id]) copy[i] = map[copy[i].id];
              }
            }
            ...
            if (options.order) {
              ...
              // Rearrange roles
              const map: Record<string, Role> = {};
              for (const role of copy) map[role.id] = role;
      ...
      mutate(
        swrErrorWrapper(
          async (domain: ExpandedDomain) => {
            ...
            return {
              ...domain,
              roles: domain.roles.filter((r) => r.id !== role_id),
            };
          },
...
export type DomainWrapper<Loaded extends boolean = true> = SwrWrapper<
  ExpandedDomain,
  Loaded,
  DomainMutators
>;
...
export function useDomain(domain_id: string | undefined) {
  return useApiQuery(
    ...
    },
    {
      then: (results) => {
        // Reset default
        if (_defaults[domain_id || '']) delete _defaults[domain_id || ''];

        for (const [resource, perms] of Object.entries(
          results._permissions.permissions,
        ))
...
export function hasPermission(
  ...
  let resource_id = resource.startsWith('channels')
    ? domain.channels[resource].inherit || resource
    : cache.get(resource)?.data?.inherit || resource;
  // console.log(resource, resource_id, permission, domain._permissions.permissions)
  return (
    domain._permissions.is_admin ||
    domain._permissions.permissions[resource_id]?.has(permission)
  );
}
...
export function hasMemberPermission(
  ...
  for (const role_id of member.roles || [])
    hasAllPerms =
      hasAllPerms && domain._permissions.permissions[role_id]?.has(permission);

  // console.log(resource, resource_id, permission, domain._permissions.permissions)
  return (
    domain._permissions.is_owner ||
    (!member.is_admin && (domain._permissions.is_admin || hasAllPerms))
  );
}
...
export function hasEntry(
  ...
  return (
    domain._permissions.is_admin ||
    domain._permissions.entries.findIndex(
      (x) => x.resource === resource_id && x.role === role,
    ) >= 0
  );
}
...
export function canSetPermissions(
  domain: DomainWrapper,
  entry: { resource: string; role: string },
) {
  ...
  return (
    domain._permissions.is_admin ||
    (domain._permissions.permissions[resource_id]?.has('can_manage') &&
      domain._permissions.permissions[role]?.has('can_manage_permissions'))
  );
}
