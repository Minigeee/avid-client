...
////////////////////////////////////////////////////////////
type SwrDbQueryOptions<In, Out, Mutators extends SwrMutators> = FetcherOptions<
  any,
  Out
> &
 ...
 */
export function useDbQuery<In, Mutators extends SwrMutators = {}, Out = In>(
  key: string | undefined,
  options: SwrDbQueryOptions<In, Out, Mutators>,
) {
  ...
  const swr = useSWR<In | null>(
    ...
    (options as _CustomFetcher<In>).fetcher?.(session) ||
      fetcher((options as _DefaultFetcher).builder || (() => ''), {
        ...options,
        session,
      }),
  );
