...
export type ApiRouteOptions<P extends ApiPath> = (ApiSchema[P] extends {
  params: string[];
}
  ? Record<ApiSchema[P]['params'][number], string>
  : {}) &
  (ApiSchema[P] extends { query: any } ? ApiSchema[P]['query'] : {}) &
  (ApiSchema[P] extends { body: any } ? ApiSchema[P]['body'] : {});

/** Api path return value */
export type ApiReturn<Path extends ApiPath> = ApiSchema[Path] extends {
  return: any;
}
  ? ApiSchema[Path]['return']
  : null;

/** Message recieved from api */
export type RawMessage = Omit<Message, 'reply_to'> & {
