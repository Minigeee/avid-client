...
import { ApiPath, ApiReturn, ApiRouteOptions, ApiSchema, ApiSchemaTemplate } from '@/lib/types';
...
export type ApiReqParams<Path extends ApiPath> = Omit<ApiSchema[Path], 'params' | 'return'> & (ApiSchema[Path] extends { params: string[] } ? {
...
function _queryEncode(x: any): string {
    ...
    if (typeof x === 'object') {
		if (x === null)
			...
			str = x.map(x => _queryEncode(x)).join(',');
		else if (typeof window !== 'undefined')
			str = window.btoa(JSON.stringify(x));
		else
			str = Buffer.from(JSON.stringify(x)).toString('base64');
	}
	else if (typeof x === 'undefined')
		str = '';
	else if (typeof x === 'string')
		str = /^\w+:\w+$/.test(x) ? id(x) : x;
    else
        str = x;
...
export async function api<P extends ApiPath>(path: P, params: ApiReqParams<P>, options: { session: SessionState; throw?: boolean } & AxiosRequestConfig): Promise<ApiReturn<P>> {
	...
	p = config.domains.api + p + (_params.query ? `?${Object.entries(_params.query).filter(([k, v]) => v !== undefined).map(([k, v]) => `${k}=${_queryEncode(v)}`).join('&')}` : '');
	...
	for (let i = 0; i < 2; ++i) {
		try {
			if (method === 'get')
				results = await axios.get(p, axiosOpts);
			else if (method === 'post')
				...
				results = await axios.delete(p, axiosOpts);
			else
				assert(false, 'unimplemented api method');
