...
export function hasRepeatEvent(day: Moment, event: CalendarEvent) {
	...
	const times = [estart];
	if (estart.dayOfYear() !== eend.dayOfYear())
		times.push(eend);

	if (event.repeat && day.isSameOrAfter(estart, 'day') && (!event.repeat.end_on || day.isSameOrBefore(event.repeat.end_on))) {
		for (const t of times) {
			let contains = false;

			if (event.repeat.interval_type === 'day')
				contains = day.diff(moment(t).startOf('day'), 'days') % event.repeat.interval === 0;
			else if (event.repeat.interval_type === 'week')
				contains = moment(day).startOf('week').diff(moment(t).startOf('week'), 'weeks') % event.repeat.interval === 0 && event.repeat.week_repeat_days !== undefined && event.repeat.week_repeat_days.indexOf(day.day()) >= 0;
			else if (event.repeat.interval_type === 'month')
				contains = moment(day).startOf('month').diff(moment(t).startOf('month'), 'months') % event.repeat.interval === 0 && day.date() === t.date();
			else
				contains = moment(day).startOf('year').diff(moment(t).startOf('year'), 'years') % event.repeat.interval === 0 && day.month() === t.month() && day.date() === t.date();
 ...
 */
export function getAllRepeatEvents(start: Moment, event: CalendarEvent, mode: 'week' | 'day' = 'week') {
	...
	// Find first event
	let diff = moment(start).startOf(interval_type).diff(moment(estart).startOf(interval_type), interval_type) % interval;
	if (diff < 0) diff += interval;
	let currTime = moment(start).startOf(interval_type).add(diff === 0 ? 0 : interval - diff, interval_type);
	// console.log(start.toISOString(), diff, currTime.toISOString())


	// Range values
	const latestStart = start.isBefore(estart) ? estart : start;
	const earliestEnd = event.repeat.end_on && rangeEnd.isAfter(event.repeat.end_on) ? moment(event.repeat.end_on) : rangeEnd;
	
	// Function for adding event
	const events: (Omit<CalendarEvent, 'start' | 'end'> & { start: Moment, end: Moment })[] = [];
	...
	while (currTime.isBefore(rangeEnd)) {
		if (event.repeat.interval_type === 'day')
			addEvent(currTime);
		else if (event.repeat.interval_type === 'week' && event.repeat.week_repeat_days) {
			...
			addEvent(moment(currTime).month(estart.month()).date(estart.date()));
