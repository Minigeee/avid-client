...
export function SettingsModal(props: SettingsModalProps) {
  ...
  const flattenedTabs = useMemo(() => {
    ...
    for (const tabs of Object.values(props.tabs)) flattened.push(...tabs);
    return flattened;
  }, [props.tabs]);
  ...
  const initialTab = useMemo(() => {
    ...
    return tabId
      ? flattenedTabs.find((x) => x.value === tabId) || flattenedTabs[0]
      : flattenedTabs[0];
  }, [flattenedTabs, props.defaultTab]);
  const [tab, setTab] = useCachedState<{ value: string; label: string }>(
    `settings.${props.navkey}.tab`,
    initialTab,
    props.defaultTab ? initialTab : undefined,
  );

  // Close handlers
  useEffect(() => {
    // Reset highlight
    if (!hasUnsavedChanges && highlightUnsaved) setHighlightUnsaved(false);
      ...
      >
        ...
        >
          ...
          <ScrollArea
            sx={{ flexGrow: 1 }}
            viewportProps={{
              style: { padding: '1.0rem 1.5rem 5.0rem 1.5rem' },
            }}
          >
...
export function SettingsModalPanel(
  ...
  const elem = useMemo(
    () => (tab === props.value ? props.children : null),
    [tab, props.value],
  );
  return <>{elem}</>;
}
...
export function UnsavedChanges<T>({ form, ...props }: UnsavedChangesProps<T>) {
  ...
  return (
    ...
    >
      {(styles) => (
        <Affix
          target={context.bodyRef.current || undefined}
          position={{ bottom: '0.75rem', right: '1.0rem' }}
        >
          <Group
            spacing={8}
            w="30rem"
...
export function popUnsaved(navkey: string) {
  ...
  if (values !== undefined) delete cache[`settings.${navkey}.unsaved`];
  return values;
}
