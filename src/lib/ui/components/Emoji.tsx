...
import { emojiSearch, EmojiType } from '@/lib/utility/emoji';
...
function CategoryIcon({ category, ...props }: CategoryIconProps) {
  if (category === 'people') return <IconMoodHappy {...props} />;
  else if (category === 'nature') return <IconDog {...props} />;
  else if (category === 'foods') return <IconApple {...props} />;
  else if (category === 'activity') return <IconBallBasketball {...props} />;
  else if (category === 'places') return <IconCar {...props} />;
  else if (category === 'objects') return <IconBulb {...props} />;
  else if (category === 'symbols') return <IconMathSymbols {...props} />;
  else if (category === 'flags') return <IconFlag {...props} />;
  else return null;
}

////////////////////////////////////////////////////////////
function getCategoryLabel(category: string) {
  if (category === 'people') return 'Faces & People';
  else if (category === 'nature') return 'Animals & Nature';
  else if (category === 'foods') return 'Food & Drinks';
  else if (category === 'activity') return 'Activities';
  else if (category === 'places') return 'Travel & Places';
  else if (category === 'objects') return 'Objects';
  else if (category === 'symbols') return 'Symbols';
  else if (category === 'flags') return 'Flags';
  else return '';
}
...
////////////////////////////////////////////////////////////
const SkinSelectItem = forwardRef<
  HTMLDivElement,
...
const ItemWrapper = memo(
  ({
    ...
    data: ((style: CSSProperties) => JSX.Element)[];
    index: number;
    style: CSSProperties;
  }) => {
    ...
    () => (
      <div
        ...
        style={{
          ...
          width:
            props.emojisPerRow * (props.emojiSize + props.gutter + 8) -
            props.gutter,
        }}
      />
    ),
...
////////////////////////////////////////////////////////////
function EmojiPickerScrollArea({
  containerRef,
  scrollInfo,
  activeCategoryIdx,
  ...props
}: EmojiPickerScrollAreaProps) {
    ...
    throttle(
      ({ y }: { y: number }) => {
        ...
        if (y > scrollInfo.titlePos[idx]) {
          while (
            idx < scrollInfo.titlePos.length &&
            y > scrollInfo.titlePos[idx]
          )
            ++idx;
          --idx;
        } else {
          while (idx > 0 && y < scrollInfo.titlePos[idx]) --idx;
        }

        // Update category index if changed
        if (idx !== activeCategoryIdx) props.setActiveCategoryIdx(idx);
  ...
  return (
    <ScrollArea
      ...
      h={350}
      offsetScrollbars
      onScrollPositionChange={onScrollChange}
    >
        ...
        Object.entries(scrollInfo.categories).map(([category, pages]) => (
          <>
            <Text
              ...
              sx={(theme) => ({
                visibility:
                  scrollPos >= (scrollInfo.hideAfter[category] || 1000000)
                    ? 'hidden'
                    : undefined,
                position: 'sticky',
                height: 32,
...
export function EmojiPicker(props: EmojiPickerProps) {
  ...
  const searchPages = useMemo(() => {
    ...
    const filtered = Object.values(data.emojis)
      .filter(
        (emoji) =>
          emoji.id.indexOf(query) >= 0 ||
          emoji.keywords.findIndex((kw) => kw.indexOf(query) >= 0) >= 0 ||
          emoji.name.toLocaleLowerCase().replace(/\s+/g, '_').indexOf(query) >=
            0,
      )
      .map((x) => x.id);
    ...
    for (let i = 0; i < numPages; ++i) {
      ...
      const buttons = filtered
        .slice(i * iconsPerPage, (i + 1) * iconsPerPage)
        .map((id) => (
          <EmojiButton
            key={id}
            id={id}
  ...
  const scrollInfo = useMemo(() => {
    ...
    for (let i = 0; i < data.categories.length; ++i) {
      ...
      const size =
        Math.ceil(category.emojis.length / iconsPerRow) * (emojiSize + 8 + 8) +
        32;
              ...
              <ActionIcon
                ...
                onClick={() => {
                  ...
                  listRef.current?.scrollTo({
                    top: scrollInfo.positions[category.id],
                  });
                }}
              >
