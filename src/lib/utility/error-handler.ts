...
export function notifyError(
  ...
  if (options?.notify !== false) {
    if (error instanceof AxiosError) setExtra('error', error.config);
    else setExtra('error', error);
    ...
    // Unauthorized
      ...
      options?.bad_input
        ...
        : notifyError(error, {
            ...
            message: `${
              options?.message || 'An application error has occurred'
            }. ${config.app.support_message}`,
          });
    } else if (status >= 500) {
      ...
      options?.internal_error
        ...
        : notifyError(error, {
            ...
            message: `${
              options?.message ||
              'An internal server or database error has occurred'
            }. ${config.app.support_message}`,
          });
    } else {
...
export function errorHandler(error: any, options?: AxiosErrorHandlerOptions) {
  if (error instanceof AxiosError) {
    ...
    notifyError(error, {
      title: 'Application Error',
      message: `${options?.message || 'An application error has occurred'}. ${
        config.app.support_message
      }`,
    });
...
export type ErrorWrapperOptions = AxiosErrorHandlerOptions & {
  ...
  return ((...args: any) => {
    try {
      ...
      if (ret && typeof ret.catch === 'function') {
        // Async handler
        ret
          .catch((err: any) => handleError(err, ...args))
...
export function swrErrorWrapper(
  fn: (...args: any) => Promise<any>,
  options?: ErrorWrapperOptions,
) {
  // Propagate by default (swr reverts data on error, and we want that)
  if (options && options.propagate === undefined) options.propagate = true;
