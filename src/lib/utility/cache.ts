...
export class SyncCache<T> {
  ...
  ////////////////////////////////////////////////////////////
  constructor(invalidate_time: number = 10 * 60) {
...
export class AsyncCache<T> {
  ...
  add(key: string | string[], data: T | T[], renew: boolean = true) {
    ...
    if (Array.isArray(key)) {
      ...
      for (let i = 0; i < key.length; ++i) {
        ...
        // Make new cache objects
        if (!renew && this._data[k]) this._data[k].data = data[i] as T;
        else {
          this._data[k] = {
            data: data[i] as T,
            last_updated: now,
      ...
      // Make new cache object
      if (!renew && this._data[key]) this._data[key].data = data as T;
      else {
        this._data[key] = {
          data: data as T,
          last_updated: now,
  ...
  async get<K extends string | string[]>(
    ...
    for (let i = 0; i < keys.length; ++i) {
      ...
      if (
        !object ||
        elapsed > this.invalidate_time ||
        (revalidate && elapsed > 1 * 60)
      )
        missing.push([k, i]);
