 ...
 */
export function remap<T, From extends keyof T, To extends string>(
  arr: T[],
  from: From,
  to: To,
): (T & { [_ in To]: T[From] })[] {
  return arr.map((x) => ({ [to]: x[from], ...x })) as (T & {
    [_ in To]: T[From];
  })[];
}
 ...
 */
export function sortObject<T extends Record<any, any>>(
  x: T,
  compareFn?: (a: [string, T[keyof T]], b: [string, T[keyof T]]) => number,
): T {
  // @ts-ignore
  return Object.entries(x)
    .sort(compareFn || (([a], [b]) => a.localeCompare(b)))
    .reduce((obj: any, [k, v]: [string, any]) => {
      obj[k] = v;
      return obj;
    }, {});
}
 ...
 */
export function diff<B extends any>(
  origObj: any,
  newObj: B,
): Partial<B> | undefined {
