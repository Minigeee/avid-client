  ...
  .post(
    ...
    expressWrapper(
      upload(
        ...
        { fileSize: _config.upload.profile_picture.max_size },
      ).single('image'),
    ),
    ...
    async (req, res) => {
      ...
      // Delete old image
      if (results[0].icon) s3.delete(getImageKey(results[0].icon));
